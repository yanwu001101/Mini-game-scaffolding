...
export default class AdSdkRyRpk extends AdSDK {
    ...
    dataLoadComplete() {
        this.adData = this.m_adSdkData.ryRpk;
        ...
        console.log("ryRpk广告参数：", + JSON.stringify(this.adData));
    }
    ...
    showInsert() {
        // console.log("显示hw插屏:", this.m_insertId);
        ...
        // this.m_insertAd = qg.createInterstitialAd(adParam);
        // if (this.m_insertAd) {
    ...
    onInsertError(err: any) {
        console.log("华为插屏错误：", JSON.stringify(err));
        this.onInsertClose();
    ...
    loadRewardVideo() {
        ...
        this.m_rewardedVideoAd.onError(err => {
            console.log("hwRpk 激励视频 广告重新加载失败" + err.code + err.msg);
            this.m_loadRewardVideoAdSuccess = false;
            Laya.timer.clear(this, this.loadRewardVideo);
            Laya.timer.once(10000, this, this.loadRewardVideo);
        })
    ...
    onNativeError(err) {
        console.log("hwRpk banner error:", JSON.stringify(err));
        this.m_nativeIndex++;
